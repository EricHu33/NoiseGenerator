        ...
        Pass
        {
            ...
            float fbm(float2 pos) {
                ...
                for (int i = 0; i < floor(_FbmStep); ++i) {
                    factor *= 0.5;
                    total += factor * lerp(snoise(pos), psnoise(pos, floor(pow(2,_Periodic)).xx), _Tilable);
                    pos *= 2.0;
                }
                return total;
            }
            ...
            {
                float2 st = i.vertex.xy / pow(2, floor(_Scale)) + _EditorTime;
                st += _Offset;
                if(_FbmEnabled > 0)
                {
                    return 0.5 + 0.5 * fbm(st);
                }
                return 0.5 + 0.5 * lerp(snoise(st), psnoise(st, floor(pow(2,_Periodic)).xx), _Tilable);
