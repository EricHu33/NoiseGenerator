Shader "Unlit/WorleyVariant2Noise"
{
   ...
   Properties
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Library/hardVoronoi.cginc"
            ...
            float fbm(float2 pos, float2 p) {
                float total = 0.0;
                float factor = 1.0;
                for (int i = 0; i < floor(_FbmStep); ++i) {
                    factor *= 0.5;
                    float2 w = voronoise( pos, p.x, p.y , lerp(float2(0,0), _Periodic.xx, _Tilable));
                    float noise = w.x;
                    total += factor * noise;
                    pos *= 2.0;
                }
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float f = voronoise( st, p.x, p.y , lerp(float2(0,0), _Periodic.xx, _Tilable));
                return f;
            }
            ENDCG
        }
